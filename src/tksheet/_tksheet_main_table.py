...
class MainTable(tk.Canvas):
    def __init__(self,
        ...
        self.default_hh = (header_height if isinstance(header_height, str) else "pixels",
                           header_height if isinstance(header_height, int) else self.GetHdrLinesHeight(
                               int(header_height)))
        self.set_fnt_help()
        ...
        if self.anything_selected():
                        ...
                        for c in range(c1, c2):
                            ...
                            if (
                                    ((data_ref_rn, dcol) in self.cell_options and (
                                            'readonly' in self.cell_options[(data_ref_rn, dcol)] or 'checkbox' in
                                            self.cell_options[(data_ref_rn, dcol)])) or
            ...
            if self.undo_enabled:
                self.undo_storage.append(
                    zlib.compress(pickle.dumps(("edit_cells", undo_storage, tuple(boxes.items()), currently_selected))))
            self.clipboard_clear()
            self.clipboard_append(s.getvalue())
            self.update()
            self.refresh()
    ...
    def ctrl_v(self, event=None):
        ...
        if self.expand_sheet_if_paste_too_big:
            ...
            if x1 + numcols > len(self.col_positions) - 1:
                ...
                if isinstance(self.paste_insert_column_limit, int) and self.paste_insert_column_limit < len(
                        self.col_positions) - 1 + added_cols:
            ...
            if y1 + numrows > len(self.row_positions) - 1:
                ...
                if isinstance(self.paste_insert_row_limit, int) and self.paste_insert_row_limit < len(
                        self.row_positions) - 1 + added_rows:
        ...
        if self.extra_begin_ctrl_v_func is not None or self.extra_end_ctrl_v_func is not None:
            rows = [[data[ndr][ndc] for ndc, c in enumerate(range(x1, x1 + numcols))] for ndr, r in
                    enumerate(range(y1, y1 + numrows))]
        ...
        if self.undo_enabled:
            self.undo_storage.append(zlib.compress(pickle.dumps(("edit_cells_paste",
                                                                 undo_storage,
                                                                 (((y1, x1, y1 + numrows, x1 + numcols), "cells"),),
                                                                 # boxes
                                                                 currently_selected,
                                                                 added_rows_cols))))
        ...
        self.see(r=y1, c=x1, keep_yscroll=False, keep_xscroll=False, bottom_right_corner=False,
                 check_cell_visibility=True, redraw=False)
        self.refresh()
    ...
    def delete_key(self, event=None):
        if self.anything_selected():
            ...
            if self.undo_enabled:
                undo_storage = {}
            boxes = []
            for item in chain(self.find_withtag("CellSelectFill"), self.find_withtag("RowSelectFill"),
                              self.find_withtag("ColSelectFill"), self.find_withtag("Current_Outside")):
                alltags = self.gettags(item)
                    ...
                    for c in range(c1, c2):
                        ...
                        if (
                                ((r, dcol) in self.cell_options and (
                                        'readonly' in self.cell_options[(r, dcol)] or 'checkbox' in self.cell_options[
                                    (r, dcol)])) or
                                (dcol in self.col_options and 'readonly' in self.col_options[dcol]) or
                                (r in self.row_options and 'readonly' in self.row_options[r])
                        ):
            ...
            if undo_storage[0] in ("edit_cells", "edit_cells_paste"):
                ...
                if undo_storage[3]:
                    if isinstance(undo_storage[3][0], int):
                        self.create_current(undo_storage[3][0], undo_storage[3][1], type_="cell",
                                            inside=True if self.cell_selected(undo_storage[3][0],
                                                                              undo_storage[3][1]) else False)
                    ...
                    self.create_current(start_row, start_col, type_="cell",
                                        inside=True if self.cell_selected(start_row, start_col) else False)
                self.see(r=start_row, c=start_col, keep_yscroll=False, keep_xscroll=False, bottom_right_corner=False,
                         check_cell_visibility=True, redraw=False)

            elif undo_storage[0] == "move_rows":
                rhs = [int(b - a) for a, b in
                       zip(self.row_positions, islice(self.row_positions, 1, len(self.row_positions)))]
                ...
                self.see(r=orig_ins_row, c=0, keep_yscroll=False, keep_xscroll=True, bottom_right_corner=False,
                         check_cell_visibility=True, redraw=False)
                ...
                self.RI.cell_options = {t1 if t1 < rm1start else t1 - totalrows: t2 for t1, t2 in
                                        self.RI.cell_options.items()}
                self.RI.cell_options = {t1 if t1 < ins_row else t1 + totalrows: t2 for t1, t2 in
                                        self.RI.cell_options.items()}
                ...
                self.cell_options = {(t10 if t10 < rm1start else t10 - totalrows, t11): t2 for (t10, t11), t2 in
                                     self.cell_options.items()}
                self.cell_options = {(t10 if t10 < ins_row else t10 + totalrows, t11): t2 for (t10, t11), t2 in
                                     self.cell_options.items()}
                ...
                cws = [int(b - a) for a, b in
                       zip(self.col_positions, islice(self.col_positions, 1, len(self.col_positions)))]
                ...
                self.see(r=0, c=orig_ins_col, keep_yscroll=True, keep_xscroll=False, bottom_right_corner=False,
                         check_cell_visibility=True, redraw=False)
                ...
                self.CH.cell_options = {t1 if t1 < rm1start else t1 - totalcols: t2 for t1, t2 in
                                        self.CH.cell_options.items()}
                self.CH.cell_options = {t1 if t1 < ins_col else t1 + totalcols: t2 for t1, t2 in
                                        self.CH.cell_options.items()}
                ...
                self.cell_options = {(t10, t11 if t11 < rm1start else t11 - totalcols): t2 for (t10, t11), t2 in
                                     self.cell_options.items()}
                self.cell_options = {(t10, t11 if t11 < ins_col else t11 + totalcols): t2 for (t10, t11), t2 in
                                     self.cell_options.items()}
                ...
                self.cell_options = {(rn if rn < idx else rn - numrows, cn): t2 for (rn, cn), t2 in
                                     self.cell_options.items()}
                ...
                if len(self.row_positions) > 1:
                    ...
                    self.see(r=start_row, c=0, keep_yscroll=False, keep_xscroll=False, bottom_right_corner=False,
                             check_cell_visibility=True, redraw=False)
                ...
                self.cell_options = {(rn, cn if cn < idx else cn - numcols): t2 for (rn, cn), t2 in
                                     self.cell_options.items()}
                ...
                if len(self.col_positions) > 1:
                    ...
                    self.see(r=0, c=start_col, keep_yscroll=False, keep_xscroll=False, bottom_right_corner=False,
                             check_cell_visibility=True, redraw=False)
    ...
    def toggle_select_cell(self, row, column, add_selection=True, redraw=True, run_binding_func=True,
                           set_as_current=True):
        if add_selection:
            if self.cell_selected(row, column, inc_rows=True, inc_cols=True):
                self.deselect(r=row, c=column, redraw=redraw)
            else:
                self.add_selection(r=row, c=column, redraw=redraw, run_binding_func=run_binding_func,
        ...
        if currently_selected[0] == "row":
            ...
            if r < len(self.row_positions) - 2 and self.RI.row_selection_enabled:
                if self.cell_is_completely_visible(r=min(r + 2, len(self.row_positions) - 2), c=0):
                    ...
                    self.RI.select_row(r + 1)
                    if r + 2 < len(self.row_positions) - 2 and (
                            self.row_positions[r + 3] - self.row_positions[r + 2]) + (
                            self.row_positions[r + 2] - self.row_positions[r + 1]) + 5 < self.winfo_height():
    ...
    def create_rc_menus(self):
        ...
        if self.cut_enabled:
            self.rc_popup_menu.add_command(label="Вырезать",
                                           accelerator="Ctrl+X",
        ...
        if self.copy_enabled:
            self.rc_popup_menu.add_command(label="Копировать",
                                           accelerator="Ctrl+C",
                                           font=self.popup_menu_font,
                                           foreground=self.popup_menu_fg,
                                           background=self.popup_menu_bg,
        ...
        if self.paste_enabled:
            self.rc_popup_menu.add_command(label="Вставить",
                                           accelerator="Ctrl+V",
                                           font=self.popup_menu_font,
                                           foreground=self.popup_menu_fg,
                                           background=self.popup_menu_bg,
        ...
        if self.delete_key_enabled:
            self.rc_popup_menu.add_command(label="Удалить",
                                           accelerator="Del",
                                           font=self.popup_menu_font,
                                           foreground=self.popup_menu_fg,
                                           background=self.popup_menu_bg,
        ...
        if self.rc_delete_row_enabled:
            self.RI.ri_rc_popup_menu.add_command(label="Удалить поле",
                                                 font=self.popup_menu_font,
                                                 foreground=self.popup_menu_fg,
                                                 background=self.popup_menu_bg,
                                                 activebackground=self.popup_menu_highlight_bg,
    ...
    def mouse_motion(self, event):
        if (
            ...
            if self.RI.width_resizing_enabled and not mouse_over_resize:
                try:
                    x1, y1, x2, y2 = self.row_width_resize_bbox[0], self.row_width_resize_bbox[1], \
                                     self.row_width_resize_bbox[2], self.row_width_resize_bbox[3]
            ...
            if self.CH.height_resizing_enabled and not mouse_over_resize:
                try:
                    x1, y1, x2, y2 = self.header_height_resize_bbox[0], self.header_height_resize_bbox[1], \
                                     self.header_height_resize_bbox[2], self.header_height_resize_bbox[3]
                    if x >= x1 and y >= y1 and x <= x2 and y <= y2:
    ...
    def b1_press(self, event=None):
        ...
        if self.identify_col(x=event.x, allow_end=False) is None or self.identify_row(y=event.y,
                                                                                      allow_end=False) is None:
            self.deselect("all")
        r = self.identify_row(y=event.y)
        c = self.identify_col(x=event.x)
        if self.single_selection_enabled and all(
        ...
        if self.drag_selection_enabled and all(
            ...
            if rowsel < len(self.row_positions) - 1 and colsel < len(self.col_positions) - 1:
                ...
                if self.shift_selection_binding_func is not None:
                    self.shift_selection_binding_func(SelectionBoxEvent("shift_select_cells", tuple(
                        int(e) for e in self.gettags(self.find_withtag("CellSelectFill"))[1].split("_") if e)))
    ...
    def b1_motion(self, event):
        ...
        if self.drag_selection_enabled and all(
            ...
            currently_selected = self.currently_selected()
            if end_row < len(self.row_positions) - 1 and end_col < len(
                    self.col_positions) - 1 and currently_selected and isinstance(currently_selected[0], int):
                ...
                if self.being_drawn_rect != rect:
                    ...
                    if self.drag_selection_binding_func is not None:
                        self.drag_selection_binding_func(SelectionBoxEvent("drag_select_cells", tuple(
                            int(e) for e in self.gettags(self.find_withtag("CellSelectFill"))[1].split("_") if e)))
            scrolled = False
            if self.data_ref:
                xcheck = self.xview()
    ...
    def GetCellCoords(self, event=None, r=None, c=None, sel=False):
        if event is not None:
            ...
            if sel:
                return self.col_positions[c] + 1, self.row_positions[r] + 1, self.col_positions[c + 1], \
                       self.row_positions[r + 1]
            else:
                return self.col_positions[c], self.row_positions[r], self.col_positions[c + 1], self.row_positions[
                    r + 1]
    ...
    def set_fnt_help(self):
        ...
        if self.default_rh[0] != "pixels":
            self.default_rh = (self.default_rh[0] if self.default_rh[0] != "pixels" else "pixels",
                               self.GetLinesHeight(int(self.default_rh[0])) if self.default_rh[0] != "pixels" else
                               self.default_rh[1])
        self.set_min_cw()
    ...
    def set_hdr_fnt_help(self):
        ...
        if self.default_hh[0] != "pixels":
            self.default_hh = (self.default_hh[0] if self.default_hh[0] != "pixels" else "pixels",
                               self.GetHdrLinesHeight(int(self.default_hh[0])) if self.default_hh[0] != "pixels" else
                               self.default_hh[1])
        self.set_min_cw()
    ...
    def set_cell_size_to_text(self, r, c, only_set_if_too_small=False, redraw=True, run_binding=False):
        ...
        if txt:
            itmcon(x, text=txt)
            b = itmbbx(x)
            tw = b[2] - b[0] + self.txt_h + 3 if (rn, cn) in self.cell_options and 'dropdown' in self.cell_options[
        ...
        if cell_needs_resize_w:
            ...
            self.col_positions[c + 2:] = [e + increment for e in
                                          islice(self.col_positions, c + 2, len(self.col_positions))]
        ...
        if cell_needs_resize_h:
            ...
            self.row_positions[r + 2:] = [e + increment for e in
                                          islice(self.row_positions, r + 2, len(self.row_positions))]
        ...
        for cn in iterable:
            ...
            for rn, r in enumerate(self.data_ref):
                ...
                if txt:
                    itmcon(x, text=txt)
                    b = itmbbx(x)
                    tw = b[2] - b[0] + self.txt_h + 3 if (rn, cn) in self.cell_options and 'dropdown' in \
                                                         self.cell_options[(rn, cn)] else b[2] - b[0] + 5
                    h = b[3] - b[1] + 5
                else:
                    if (rn, cn) in self.cell_options and 'dropdown' in self.cell_options[(rn, cn)]:
                        tw = self.txt_h + 3
    ...
    def insert_col_rc(self, event=None):
        if self.anything_selected(exclude_rows=True, exclude_cells=True):
            ...
            data_ins_col = int(displayed_ins_col)
        if isinstance(self.paste_insert_column_limit,
                      int) and self.paste_insert_column_limit < displayed_ins_col + numcols:
        ...
        if not self.all_columns_displayed:
            if displayed_ins_col == len(self.col_positions) - 1:
                ...
                part3 = [] if displayed_ins_col > len(self.displayed_columns) - 1 else [cn + numcols for cn in
                                                                                        islice(self.displayed_columns,
                                                                                               adj_ins + 1, None)]
        ...
        self.insert_col_positions(idx=displayed_ins_col,
                                  widths=numcols,
                                  deselect_all=True)
        self.cell_options = {(rn, cn if cn < data_ins_col else cn + numcols): t2 for (rn, cn), t2 in
                             self.cell_options.items()}
    ...
    def insert_row_rc(self, event=None):
        ...
        self.insert_row_positions(idx=posidx,
                                  heights=numrows,
                                  deselect_all=True)
        self.cell_options = {(rn if rn < posidx else rn + numrows, cn): t2 for (rn, cn), t2 in
                             self.cell_options.items()}
        ...
        if seld_rows:
            ...
            if self.undo_enabled:
                ...
                for r in reversed(seld_rows):
                    undo_storage['deleted_rows'].append(
                        (r, self.data_ref.pop(r), self.row_positions[r + 1] - self.row_positions[r]))
            else:
                for r in reversed(seld_rows):
                    del self.data_ref[r]
            if self.my_row_index and isinstance(self.my_row_index, list):
    ...
    def display_columns(self, indexes=None, enable=None, reset_col_positions=True, set_col_positions=True,
                        deselect_all=True):
        ...
        if indexes is not None:
            ...
            if self.displayed_columns:
                self.hidd_col_expander_idxs = [c1 for c1, c2 in zip(islice(self.displayed_columns, 0, None),
                                                                    islice(self.displayed_columns, 1, None)) if
                                               c2 - c1 != 1]
    ...
    def headers(self, newheaders=None, index=None, reset_col_positions=False, show_headers_if_not_sheet=True):
        if newheaders is not None:
            ...
            if reset_col_positions:
                self.reset_col_positions()
            elif show_headers_if_not_sheet and isinstance(self.my_hdrs, list) and (
                    self.col_positions == [0] or not self.col_positions):
                colpos = int(self.default_cw)
                if self.all_columns_displayed:
    ...
    def row_index(self, newindex=None, index=None, reset_row_positions=False, show_index_if_not_sheet=True):
        if newindex is not None:
            ...
            if reset_row_positions:
                self.reset_row_positions()
            elif show_index_if_not_sheet and isinstance(self.my_row_index, list) and (
                    self.row_positions == [0] or not self.row_positions):
                rowpos = self.default_rh[1]
            ...
            if len(self.data_ref) < total_rows:
                if total_columns is None:
                    ...
                    self.data_ref.extend(
                        [list(repeat("", total_data_cols)) for r in range(total_rows - len(self.data_ref))])
                else:
                    self.data_ref.extend(
                        [list(repeat("", total_columns)) for r in range(total_rows - len(self.data_ref))])
            else:
                self.data_ref[total_rows:] = []
        if total_columns is not None:
            self.data_ref[:] = [
                r[:total_columns] if len(r) > total_columns else r + list(repeat("", total_columns - len(r))) for r in
                self.data_ref]

    def equalize_data_row_lengths(self):
        total_columns = self.total_data_cols()
        self.data_ref[:] = [r + list(repeat("", total_columns - len(r))) if total_columns > len(r) else r for r in
                            self.data_ref]
    ...
    def redraw_highlight_get_text_fg(self, r, c, fc, fr, sc, sr, c_2_, c_3_, c_4_, selected_cells, actual_selected_rows,
                                     actual_selected_cols, dcol, can_width):
        redrawn = False
        ...
        if (r, dcol) in self.cell_options and 'highlight' in self.cell_options[(r, dcol)] and (r, c) in selected_cells:
            tf = self.table_selected_cells_fg if self.cell_options[(r, dcol)]['highlight'][
                                                     1] is None or self.display_selected_fg_over_highlights else \
            self.cell_options[(r, dcol)]['highlight'][1]
            if self.cell_options[(r, dcol)]['highlight'][0] is not None:
                c_1 = self.cell_options[(r, dcol)]['highlight'][0] if self.cell_options[(r, dcol)]['highlight'][
                    0].startswith("#") else Color_Map_[self.cell_options[(r, dcol)]['highlight'][0]]
                redrawn = self.redraw_highlight(fc + 1, fr + 1, sc, sr,
                                                fill=(f"#{int((int(c_1[1:3], 16) + c_2_[0]) / 2):02X}" +
                                                      f"{int((int(c_1[3:5], 16) + c_2_[1]) / 2):02X}" +
                                                      f"{int((int(c_1[5:], 16) + c_2_[2]) / 2):02X}"),
                                                outline=self.table_fg if (
                                                                         r, dcol) in self.cell_options and 'dropdown' in
                                                                         self.cell_options[(r, dcol)] else "", tag="hi")
            ...
            tf = self.table_selected_cells_fg if self.row_options[r]['highlight'][
                                                     1] is None or self.display_selected_fg_over_highlights else \
            self.row_options[r]['highlight'][1]
            if self.row_options[r]['highlight'][0] is not None:
                c_1 = self.row_options[r]['highlight'][0] if self.row_options[r]['highlight'][0].startswith("#") else \
                Color_Map_[self.row_options[r]['highlight'][0]]
                redrawn = self.redraw_highlight(fc + 1, fr + 1, sc, sr,
                                                fill=(f"#{int((int(c_1[1:3], 16) + c_2_[0]) / 2):02X}" +
                                                      f"{int((int(c_1[3:5], 16) + c_2_[1]) / 2):02X}" +
                                                      f"{int((int(c_1[5:], 16) + c_2_[2]) / 2):02X}"),
            ...
            tf = self.table_selected_cells_fg if self.col_options[dcol]['highlight'][
                                                     1] is None or self.display_selected_fg_over_highlights else \
            self.col_options[dcol]['highlight'][1]
        ...
        elif (r, dcol) in self.cell_options and 'highlight' in self.cell_options[
            (r, dcol)] and r in actual_selected_rows:
            tf = self.table_selected_rows_fg if self.cell_options[(r, dcol)]['highlight'][
                                                    1] is None or self.display_selected_fg_over_highlights else \
            self.cell_options[(r, dcol)]['highlight'][1]
            if self.cell_options[(r, dcol)]['highlight'][0] is not None:
                c_1 = self.cell_options[(r, dcol)]['highlight'][0] if self.cell_options[(r, dcol)]['highlight'][
        ...
        # ________________________ CELL IS HIGHLIGHTED AND IN SELECTED COLUMNS ________________________
            ...
            (r, dcol)] and c in actual_selected_cols:
            tf = self.table_selected_columns_fg if self.cell_options[(r, dcol)]['highlight'][
                                                       1] is None or self.display_selected_fg_over_highlights else \
            self.cell_options[(r, dcol)]['highlight'][1]
            if self.cell_options[(r, dcol)]['highlight'][0] is not None:
                c_1 = self.cell_options[(r, dcol)]['highlight'][0] if self.cell_options[(r, dcol)]['highlight'][
        ...
        elif (r, dcol) in self.cell_options and 'highlight' in self.cell_options[(r, dcol)] and (
        r, c) not in selected_cells and r not in actual_selected_rows and c not in actual_selected_cols:
            tf = self.table_fg if self.cell_options[(r, dcol)]['highlight'][1] is None else \
            self.cell_options[(r, dcol)]['highlight'][1]
            if self.cell_options[(r, dcol)]['highlight'][0] is not None:
                redrawn = self.redraw_highlight(fc + 1, fr + 1, sc, sr,
                                                fill=self.cell_options[(r, dcol)]['highlight'][0],
                                                outline=self.table_fg if (
                                                                         r, dcol) in self.cell_options and 'dropdown' in
                                                                         self.cell_options[(r, dcol)] else "", tag="hi")

        elif r in self.row_options and 'highlight' in self.row_options[r] and (
        r, c) not in selected_cells and r not in actual_selected_rows and c not in actual_selected_cols:
        ...
        elif dcol in self.col_options and 'highlight' in self.col_options[dcol] and (
        r, c) not in selected_cells and r not in actual_selected_rows and c not in actual_selected_cols:
            tf = self.table_fg if self.col_options[dcol]['highlight'][1] is None else \
            self.col_options[dcol]['highlight'][1]
            if self.col_options[dcol]['highlight'][0] is not None:
    ...
    def redraw_highlight(self, x1, y1, x2, y2, fill, outline, tag, can_width=None):
        if self.hidd_high:
            ...
            self.coords(t, x1 - 1 if outline else x1, y1 - 1 if outline else y1,
                        x2 if can_width is None else x2 + can_width, y2)
            if sh:
            ...
            t = self.create_rectangle(x1 - 1 if outline else x1, y1 - 1 if outline else y1,
                                      x2 if can_width is None else x2 + can_width, y2, fill=fill, outline=outline,
                                      tag=tag)
        ...
        if draw_check:
            ...
            if self.hidd_grid:
                ...
                if sh:
                    ...
                    self.itemconfig(t, fill=self.table_bg, capstyle=tk.ROUND, joinstyle=tk.ROUND, width=2, tag=tag,
                                    state="normal")
                self.lift(t)
            else:
                t = self.create_line(x1 + 2, y1 + 2, x2 - 2, y2 - 2, fill=self.table_bg, capstyle=tk.ROUND,
                                     joinstyle=tk.ROUND, width=2, tag=tag)
            ...
            if self.hidd_grid:
                ...
                t = self.create_line(x2 - 2, y1 + 2, x1 + 2, y2 - 2, fill=self.table_bg, capstyle=tk.ROUND,
                                     joinstyle=tk.ROUND, width=2, tag=tag)
            self.disp_grid[t] = True
    ...
    def main_table_redraw_grid_and_text(self, redraw_header=False, redraw_row_index=False, redraw_table=True):
        ...
        if self.show_horizontal_grid:
            for r in range(start_row - 1, end_row):
                ...
                if self.hidd_grid:
                    ...
                    self.disp_grid[
                        self.create_line(x1, y, x2 + can_width if self.horizontal_grid_to_end_of_window else x_stop, y,
                                         fill=self.table_grid_fg, capstyle=tk.BUTT, joinstyle=tk.ROUND, width=1,
                                         tag="g")] = True
        if self.show_vertical_grid:
            for c in range(start_col - 1, end_col):
                ...
                if self.hidd_grid:
                    ...
                    self.disp_grid[
                        self.create_line(x, y1, x, y2 + can_height if self.vertical_grid_to_end_of_window else y_stop,
                                         fill=self.table_grid_fg, capstyle=tk.BUTT, joinstyle=tk.ROUND, width=1,
                                         tag="g")] = True
        if start_row > 0:
            start_row -= 1
        if start_col > 0:
            start_col -= 1
        end_row -= 1
        c_2 = self.table_selected_cells_bg if self.table_selected_cells_bg.startswith("#") else Color_Map_[
            self.table_selected_cells_bg]
        c_2_ = (int(c_2[1:3], 16), int(c_2[3:5], 16), int(c_2[5:], 16))
        c_3 = self.table_selected_columns_bg if self.table_selected_columns_bg.startswith("#") else Color_Map_[
            self.table_selected_columns_bg]
        c_3_ = (int(c_3[1:3], 16), int(c_3[3:5], 16), int(c_3[5:], 16))
        c_4 = self.table_selected_rows_bg if self.table_selected_rows_bg.startswith("#") else Color_Map_[
        ...
        if redraw_table:
            for c in range(start_col, end_col - 1):
                for r in rows_:
                    ...
                    tf, dd_drawn = self.redraw_highlight_get_text_fg(r, c, fc, fr, sc, sr, c_2_, c_3_, c_4_,
                                                                     selected_cells, actual_selected_rows,
                                                                     actual_selected_cols, dcol, can_width)
                    ...
                    if cell_alignment == "w":
                        ...
                        if (r, dcol) in self.cell_options and 'dropdown' in self.cell_options[(r, dcol)]:
                            ...
                            self.redraw_dropdown(fc, fr, sc, self.row_positions[r + 1], fill=tf, outline=tf, tag="dd",
                                                 draw_outline=not dd_drawn, draw_arrow=mw >= 5)
                        else:
                            mw = sc - fc - 5
                    ...
                    if (r, dcol) in self.cell_options and 'checkbox' in self.cell_options[(r, dcol)]:
                        if mw > self.txt_h + 2:
                            ...
                            self.redraw_checkbox(fc + 2,
                                                 ...
                                                 fc + 2 + self.txt_h + 2,
                                                 cent + self.half_txt_h + 1,
                                                 fill=tf if self.cell_options[(r, dcol)]['checkbox'][
                                                                'state'] == "normal" else self.table_grid_fg,
                    ...
                    try:
                        if cell_alignment == "w":
                            if x > x2 or mw <= 5:
                                continue
                            lns = self.data_ref[r][dcol].split("\n") if isinstance(self.data_ref[r][dcol],
                                                                                   str) else f"{self.data_ref[r][dcol]}".split(
                                "\n")
                            y = fr + self.fl_ins
                            if y + self.half_txt_h - 1 > y1:
                                ...
                                if self.hidd_text:
                                    ...
                                    t = self.create_text(x, y, text=txt, fill=tf, font=self.my_font, anchor="w",
                                                         tag="t")
                            ...
                            if y + self.half_txt_h - 1 > y1:
                                ...
                                if self.hidd_text:
                                    ...
                                    t = self.create_text(x, y, text=txt, fill=tf, font=self.my_font, anchor="e",
                                                         tag="t")
                                self.disp_text[t] = True
                                wd = self.bbox(t)
                                wd = wd[2] - wd[0]
                            ...
                            if y + self.half_txt_h - 1 > y1:
                                if self.hidd_text:
                                    ...
                                    t = self.create_text(x, y, text=txt, fill=tf, font=self.my_font, anchor="center",
                                                         tag="t")
                                self.disp_text[t] = True
                                wd = self.bbox(t)
                                wd = wd[2] - wd[0]
                                ...
                                if y + self.half_txt_h - 1 < sr:
                                    for i in range(stl, len(lns)):
                                        ...
                                        if self.hidd_text:
                                            ...
                                            self.coords(t, x, y)
                                            if sh:
                                                self.itemconfig(t, text=txt, fill=tf, font=self.my_font,
                                                                anchor="center")
                                            else:
                                                self.itemconfig(t, text=txt, fill=tf, font=self.my_font,
                                                                anchor="center", state="normal")
                                        else:
                                            t = self.create_text(x, y, text=txt, fill=tf, font=self.my_font,
                                                                 anchor="center", tag="t")
                                        self.disp_text[t] = True
                                        wd = self.bbox(t)
                                        wd = wd[2] - wd[0]
        ...
        try:
            ...
            if redraw_header and self.show_header:
                self.CH.redraw_grid_and_text(last_col_line_pos, x1, x_stop, start_col, end_col, selected_cols,
                                             actual_selected_rows, actual_selected_cols)
            if redraw_row_index and self.show_index:
                self.RI.redraw_grid_and_text(last_row_line_pos, y1, y_stop, start_row, end_row + 1, y2, x1, x_stop,
                                             selected_rows, actual_selected_cols, actual_selected_rows)
    ...
    def get_all_selection_items(self):
        return sorted(self.find_withtag("CellSelectFill") + self.find_withtag("RowSelectFill") + self.find_withtag(
            "ColSelectFill") + self.find_withtag("Current_Inside") + self.find_withtag("Current_Outside"))
    ...
    def reselect_from_get_boxes(self, boxes):
        for k, v in boxes.items():
            ...
            elif v in ("cell_inside", "cell_outside", "row_inside", "row_outside", "col_outside",
                       "col_inside"):  # currently selected
                x = v.split("_")
    ...
    def create_current(self, r, c, type_="cell", inside=False):  # cell, col or row
        ...
        if self.show_selected_cells_border:
            b = self.create_rectangle(self.col_positions[c1] + 1, self.row_positions[r1] + 1, self.col_positions[c2],
            ...
            b = self.create_rectangle(self.col_positions[c1], self.row_positions[r1], self.col_positions[c2],
                                      self.row_positions[r2],
    ...
    def set_current_to_last(self):
        if not self.currently_selected():
            items = sorted(self.find_withtag("CellSelectFill") + self.find_withtag("RowSelectFill") + self.find_withtag(
                "ColSelectFill"))
            if items:
    ...
    def create_selected(self, r1=None, c1=None, r2=None, c2=None, type_="cells", taglower=True):
        ...
        r = self.create_rectangle(self.col_positions[c1],
                                  ...
                                  self.canvasx(self.winfo_width()) if self.selected_rows_to_end_of_window else
    ...
    def get_selected_cells(self, get_rows=False, get_cols=False, within_range=None):
        ...
        if get_cols and get_rows:
            iterable = chain(self.find_withtag("CellSelectFill"), self.find_withtag("RowSelectFill"),
                             self.find_withtag("ColSelectFill"), self.find_withtag("Current_Outside"))
        elif get_rows and not get_cols:
            iterable = chain(self.find_withtag("CellSelectFill"), self.find_withtag("RowSelectFill"),
                             self.find_withtag("Current_Outside"))
        elif get_cols and not get_rows:
            iterable = chain(self.find_withtag("CellSelectFill"), self.find_withtag("ColSelectFill"),
                             self.find_withtag("Current_Outside"))
        else:
            iterable = chain(self.find_withtag("CellSelectFill"), self.find_withtag("Current_Outside"))
        if within_range is None:
    ...
    def get_all_selection_boxes(self):
        return tuple(tuple(int(e) for e in self.gettags(item)[1].split("_") if e) for item in
                     chain(self.find_withtag("CellSelectFill"),
                           self.find_withtag("RowSelectFill"),
                           self.find_withtag("ColSelectFill"),
                           self.find_withtag("Current_Outside")))
    ...
    def get_all_selection_boxes_with_types(self):
        boxes = []
        for item in sorted(self.find_withtag("CellSelectFill") + self.find_withtag("RowSelectFill") + self.find_withtag(
    ...
    def cell_selected(self, r, c, inc_cols=False, inc_rows=False):
        if not inc_cols and not inc_rows:
            iterable = chain(self.find_withtag("CellSelectFill"), self.find_withtag("Current_Inside"),
                             self.find_withtag("Current_Outside"))
        elif inc_cols and not inc_rows:
            iterable = chain(self.find_withtag("ColSelectFill"), self.find_withtag("CellSelectFill"),
                             self.find_withtag("Current_Inside"), self.find_withtag("Current_Outside"))
        elif not inc_cols and inc_rows:
            iterable = chain(self.find_withtag("RowSelectFill"), self.find_withtag("CellSelectFill"),
                             self.find_withtag("Current_Inside"), self.find_withtag("Current_Outside"))
        elif inc_cols and inc_rows:
            iterable = chain(self.find_withtag("RowSelectFill"), self.find_withtag("ColSelectFill"),
                             self.find_withtag("CellSelectFill"), self.find_withtag("Current_Inside"),
                             self.find_withtag("Current_Outside"))
        for item in iterable:
            r1, c1, r2, c2 = tuple(int(e) for e in self.gettags(item)[1].split("_") if e)
    ...
    def anything_selected(self, exclude_columns=False, exclude_rows=False, exclude_cells=False):
        if exclude_columns and exclude_rows and not exclude_cells:
            ...
            if self.find_withtag("CellSelectFill") or self.find_withtag("RowSelectFill") or self.find_withtag(
                    "Current_Outside"):
                return True
        elif exclude_rows and not exclude_columns and not exclude_cells:
            if self.find_withtag("CellSelectFill") or self.find_withtag("ColSelectFill") or self.find_withtag(
                    "Current_Outside"):
                return True
            ...
            if self.find_withtag("CellSelectFill") or self.find_withtag("RowSelectFill") or self.find_withtag(
                    "ColSelectFill") or self.find_withtag("Current_Outside"):
                return True
        return False
    ...
    def open_cell(self, event=None):
        ...
        if (
            ...
            return
        elif (y1, dcol) in self.cell_options and (
                'dropdown' in self.cell_options[(y1, dcol)] or 'checkbox' in self.cell_options[(y1, dcol)]):
    ...
    def edit_cell_(self, event=None, r=None, c=None, dropdown=False):
        ...
        if event is not None and event.keycode == 8:  # backspace
            ...
            text = ""
              ...
              (event is not None and hasattr(event, 'keycode') and event.keycode == "??" and hasattr(event,
                                                                                                     'num') and event.num == 1) or
              event is None
        ):
            if event is not None:
                if event.keycode == 13:
    ...
    def create_text_editor(self,
        ...
        self.text_editor = TextEditor(self, text=text, font=self.my_font, state=state, width=w, height=h,
                                      ...
                                      show_border=self.show_selected_cells_border, bg=bg, fg=fg)
    ...
    # c is displayed col
    def get_text_editor_value(self, destroy_tup=None, r=None, c=None, set_data_ref_on_destroy=True, event=None,
                              destroy=True, move_down=True, redraw=True, recreate=True):
        ...
        if set_data_ref_on_destroy:
            ...
            if self.extra_end_edit_cell_func is not None:
                self.extra_end_edit_cell_func(
                    EditCellEvent(r, c, destroy_tup[2] if len(destroy_tup) >= 3 else "FocusOut",
                                  f"{self.text_editor_value}", "end_edit_cell"))
        if move_down:
            if r is None and c is None and destroy_tup:
                r, c = destroy_tup[0], destroy_tup[1]
            currently_selected = self.currently_selected()
            if r is not None and c is not None:
                if (
                    ...
                    if destroy_tup is not None and len(destroy_tup) >= 3 and destroy_tup[2] == "Return":
                        ...
                        self.see(r + 1 if r < len(self.row_positions) - 2 else r, c, keep_xscroll=True,
                        ...
                        self.see(r, c + 1 if c < len(self.col_positions) - 2 else c, keep_xscroll=True,
                                 bottom_right_corner=True, check_cell_visibility=True)
        self.hide_dropdown_window(r, c)
        if recreate:
            self.recreate_all_selection_boxes()
    ...
    def _click_checkbox(self, r, c, dcol=None, undo=True, redraw=True):
        ...
        if self.cell_options[(r, dcol)]['checkbox']['state'] == "normal":
            self._set_cell_data(r, c, dcol,
                                value=not self.data_ref[r][dcol] if type(self.data_ref[r][dcol]) == bool else False,
                                undo=undo, cell_resize=False)
            self.cell_options[(r, dcol)]['checkbox']['check_function'](
                (r, c, "CheckboxClicked", f"{self.data_ref[r][dcol]}"))
        if redraw:
            self.refresh()
    ...
    def create_checkbox(self, r=0, c=0, checked=False, state="normal", redraw=False, check_function=None):
        ...
        self._set_cell_data(r, dcol=c, value=checked, cell_resize=False,
                            undo=False)  # only works because cell_resize is false and undo is false, otherwise needs displayed col and dcol args
        if (r, c) not in self.cell_options:
            self.cell_options[(r, c)] = {}
    ...
    def create_dropdown(self, r=0, c=0, values=[], set_value=None, state="readonly", redraw=True,
                        selection_function=None, modified_function=None):
        if (r, c) in self.cell_options and any(x in self.cell_options[(r, c)] for x in ('dropdown', 'checkbox')):
            self.destroy_dropdown_and_checkbox(r, c)
        if values:
            self._set_cell_data(r, c, value=set_value if set_value is not None else values[0], cell_resize=False,
                                undo=False)
        ...
        if self.cell_options[(r, dcol)]['dropdown']['state'] == "normal":
            ...
            if self.cell_options[(r, dcol)]['dropdown']['modified_function'] is not None:
                self.text_editor.textedit.bind("<<TextModified>>",
                                               self.cell_options[(r, dcol)]['dropdown']['modified_function'])
            self.update()
            try:
                self.text_editor.textedit.focus_set()
                self.text_editor.scroll_to_bottom()
    ...
    def hide_dropdown_window(self, r=None, c=None, selection=None, b1=False, redraw=True):
        if r is not None and c is not None and selection is not None:
            dcol = c if self.all_columns_displayed else self.displayed_columns[c]
            if self.cell_options[(r, dcol)]['dropdown'][
                'select_function'] is not None:  # user has specified a selection function
    ...
    def delete_opened_dropdown_window(self, r=None, c=None, dcol=None):
        ...
        try:
            ...
            pass
        self.existing_dropdown_window = None
        if r is not None and c is not None and (r, dcol) in self.cell_options and 'dropdown' in self.cell_options[
            (r, dcol)]:
